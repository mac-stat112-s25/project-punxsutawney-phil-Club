...
```{r}
library(readr)
confusionmat <- groundhogs_final |> 
  select(earlyspring, shadow) |> 
  mutate(invertspring = !earlyspring)

in_sample_confusion <- confusionmat %>% 
  conf_mat(truth = invertspring, estimate = shadow)
```
